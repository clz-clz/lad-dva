...
from multi_agent_v2 import _weighted_majority_voting, DATASET_ENTITY_TYPES
...
CONSENSUS_RATIO = 0.7   # matches verifier_node base-vote
...
def _decode_no_verifier(row: dict, dirty: List[str], valid_set: set,
                        policy: str) -> List[str]:
    """Reproduce verifier_node's output when use_verifier=False (base vote)."""
    ...
    base = _weighted_majority_voting(cands, weights, entity_boost=1.0,
                                     dirty_tags=dirty, consensus_ratio=CONSENSUS_RATIO)
...
def derive_from_candidates(source_config: str, target_config: str,
    ...
    out_rows = [{"tokens": r["tokens"], "gold_tags": r["gold_tags"],
                 "pred_tags": _decode_no_verifier(r, dirty[i], valid_set, policy)}
...
def main(argv=None) -> int:
    ...
    ap.add_argument("which", choices=["no_verifier", "no_deanchor_btif", "vote", "all"])
